...
static bool TraceToExit(Vector& end, trace_t* enter_trace, Vector start, Vector dir, int target, trace_t* exit_trace)
{
	float distance = 0.0f;
	...
	while (distance <= 90.0f)
	{
		...
		if (!(exit_trace->fraction < 1.0 || exit_trace->allsolid || exit_trace->startsolid) || exit_trace->startsolid)
		{
			...
			{
				if (enter_trace->m_pEntityHit && enter_trace->m_pEntityHit == entityList->GetClientEntity(target))
					return true;
			}
...
static bool HandleBulletPenetration(CCSWeaponInfo* weaponInfo, int target, Autowall::FireBulletData& data)
{
	...
	if (!TraceToExit(dummy, &data.enter_trace, data.enter_trace.endpos, data.direction, target, &trace_exit))
		return false;
...
bool SimulateFireBullet(C_BaseCombatWeapon* pWeapon, int target, bool teamCheck, Autowall::FireBulletData& data)
{
	...
	while (data.penetrate_count > 0 && data.current_damage >= 1.0f)
	{
		...
		if (!HandleBulletPenetration(weaponInfo, target, data))
			break;
	}
...
float Autowall::GetDamage(const Vector& point, int target, bool teamCheck, FireBulletData& fData)
{
	float damage = 0.f;
	Vector dst = point;
	C_BasePlayer* localplayer = (C_BasePlayer*) entityList->GetClientEntity(engine->GetLocalPlayer());
	...
	if (SimulateFireBullet(activeWeapon, target, teamCheck, data))
...
float Autowall::GetDamageCustom(const Vector& vecPoint, const Vector& local, int target, bool teamCheck, FireBulletData& fData)
{
	float damage = 0.f;
	Vector dst = vecPoint;
	C_BasePlayer* localplayer = (C_BasePlayer*) entityList->GetClientEntity(engine->GetLocalPlayer());
