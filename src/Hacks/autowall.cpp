...
static bool TraceToExit(Vector& end, trace_t* enter_trace, Vector start, Vector dir, trace_t* exit_trace, int playerid)
{
    float distance = 0.0f;
    ...
    while (distance <= 90.0f)
    {
        ...
        trace->TraceRay(ray, MASK_SHOT | CONTENTS_GRATE, 0, exit_trace);
        ...
        if (!(exit_trace->fraction < 1.0 || exit_trace->allsolid || exit_trace->startsolid) || exit_trace->startsolid)
        {
            ...
            {
                if (enter_trace->m_pEntityHit && enter_trace->m_pEntityHit == entityList->GetClientEntity(playerid))
                    return true;
            }
...
static bool HandleBulletPenetration(CCSWeaponInfo* weaponInfo, Autowall::FireBulletData& data, int playerid)
{
    ...
    if (!TraceToExit(dummy, &data.enter_trace, data.enter_trace.endpos, data.direction, &trace_exit, playerid))
        return false;
    ...
    float invPenMod = std::fmaxf(1.f / combined_penetration_modifier, 0.f);

    float lost_damage = std::fmaxf((invPenMod * thickness * (1.f / 24.f)) + ((data.current_damage * final_damage_modifier) + std::fmaxf(3.75f / weaponInfo->GetPenetration(), 0.f) * 3.f * invPenMod), 0.f);
...
bool SimulateFireBullet(C_BasePlayer* ent, C_BaseCombatWeapon* pWeapon, bool teamCheck, Autowall::FireBulletData& data, int playerid)
{
    C_BasePlayer* localplayer = ent;//(C_BasePlayer*) entityList->GetClientEntity(engine->GetLocalPlayer());
    CCSWeaponInfo* weaponInfo = pWeapon->GetCSWpnData();
    if(!weaponInfo)
        return false;

    if(playerid == -1)
        playerid = Aimbot::targetAimbot;
    ...
    while (data.penetrate_count > 0 && data.current_damage >= 1.0f)
    {
        ...
        if (!HandleBulletPenetration(weaponInfo, data, playerid))
            break;
    }
...
float Autowall::GetDamage(const Vector& point, bool teamCheck, FireBulletData& fData, int playerid)
{
    ...
    if (SimulateFireBullet(localplayer, activeWeapon, teamCheck, data, playerid))
...
float Autowall::GetDamageCustom(C_BasePlayer* player, Vector eyePos, const Vector& point, bool teamCheck, FireBulletData& fData, int playerid)
{
    if(!player)
        return -1.0f;
    ...
    if (SimulateFireBullet(player, activeWeapon, teamCheck, data, playerid))
        damage = data.current_damage;
